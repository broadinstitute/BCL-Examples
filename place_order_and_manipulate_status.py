...
new_status = "RECEIVED"
update_sample_status(order_key=order_id, sample_ids=samples, test_code=test_code, email=submitter, status=new_status,
                     session=auth_session)

find_order = retrieve_gpo_order(email=submitter, order_key=order_id, session=auth_session).json()
...
for test in find_order["tests"]:
    for sample in test["test_samples"]:
        if sample["sidr_sample_id"] in samples:
            ...
            click.echo(f"Verified that sample {sample['sidr_sample_id']} had its status modified to {new_status}")
